...
use Exception;
use App\Http\Requests\PartnersFormRequest;
...
class PartnersController extends Controller
{
    ...
    public function index(){
        return  view('Dashboard.Pages.Admin_partners_manage');
   }

   public function  GetAllData(){
      $query = PartnersModel::select('id','image_type','image');
...
public function SaveMaster(PartnersFormRequest $req){
...
//   INSERT DATA
public function insert(PartnersFormRequest $req){
$date = date("Y-m-d");
if($req->file('image')){
...
$IMAGE_PATH = "/upload/partners/image/";
...
$InsertData = PartnersModel::insert($data);
if($InsertData){
...
// UPDATE TEAM
public function update(PartnersFormRequest $req){
$id = $req->input('id');
if($id){
$date = date("Y-m-d");
if($req->file('image')){
...
$getImageData = PartnersModel::find($id);
...
$UpdateData = PartnersModel::where('id',$id)->update($dataArray);
if($UpdateData){
...
try{
if($request->input('id')){
$GetDataById = PartnersModel::find($request->input('id'));
if($GetDataById){
...
try{
$findById = PartnersModel::find($req->input('id'));
if($findById){
